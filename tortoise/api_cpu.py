...
def format_conditioning(clip, cond_length=132300, device="cpu"):
    """
...
def do_spectrogram_diffusion(diffusion_model, diffuser, latents, conditioning_latents, temperature=1, verbose=True):
    ...
    with torch.no_grad():
        ...
        noise = torch.randn(output_shape) * temperature
...
def pick_best_batch_size_for_cpu():
    """
    Picks a batch size for CPU. This is a simplified version that always returns 1.
    """
    return 1
...
class TextToSpeech:
    ...
    def __init__(self, autoregressive_batch_size=None, models_dir=MODELS_DIR, 
                 enable_redaction=True, kv_cache=False, use_deepspeed=False, half=False,
                 tokenizer_vocab_file=None, tokenizer_basic=False):

        """
        Constructor
        :param autoregressive_batch_size: Specifies how many samples to generate per batch. Lower this if you are seeing
                                          memory errors. Larger numbers generates slightly faster.
        ...
        self.autoregressive_batch_size = pick_best_batch_size_for_cpu() if autoregressive_batch_size is None else autoregressive_batch_size
    ...
    def load_cvvp(self):
        ...
        self.cvvp.load_state_dict(torch.load(get_model_path('cvvp.pth', self.models_dir), map_location=torch.device('cpu')))
    ...
    def get_conditioning_latents(self, voice_samples, return_mels=False):
        ...
        """
        with torch.no_grad():
            voice_samples = [v.cpu() for v in voice_samples]
            ...
            if self.stft is None:
                # Initialize STFT
                self.stft = TacotronSTFT(1024, 256, 1024, 100, 24000, 0, 12000).cpu()
            ...
            for sample in voice_samples:
                ...
                cond_mel = wav_to_univnet_mel(sample, do_normalization=False, device='cpu', stft=self.stft)
    ...
    def tts_with_preset(self, text, preset='ultra_fast', **kwargs):
        ...
        presets = {
             'fastest' : {'num_autoregressive_samples': 1, 'diffusion_iterations': 5, 'cond_free': False},
    ...
    def tts(self, text, voice_samples=None, conditioning_latents=None, k=1, verbose=True, use_deterministic_seed=None, return_deterministic_state=False,
            ...
            cvvp_amount=.0,
            diffusion_iterations=100, cond_free=True, cond_free_k=2, diffusion_temperature=1.0, **hf_generate_kwargs):
        """
        ...
        with torch.no_grad():
            ...
            for b in tqdm(range(num_batches), disable=not verbose):
                codes = self.autoregressive.inference_speech(auto_conditioning, text_tokens, do_sample=True,
            ...
            for b in range(best_results.shape[0]):
                ...
                latents = self.autoregressive(auto_conditioning.repeat(1, 1), text_tokens.repeat(1, 1),
    ...
    def deterministic_state(self, seed=None):
        ...
        random.seed(seed)
        # torch.use_deterministic_algorithms(True) の行は削除しました
